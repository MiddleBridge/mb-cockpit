...
export default function GeneralNotesView() {
  ...
  useEffect(() => {
    async function loadNote() {
      setIsLoading(true);
      const savedNote = await generalNotesDb.getGeneralNote();
      if (savedNote) {
        setTitle(savedNote.title || "");
        setContent(savedNote.content || "");
        setSource(savedNote.source || "");
  ...
  const handleSave = async () => {
    setIsSaving(true);
    const success = await generalNotesDb.saveGeneralNote({
      title,
      content,
      source
    });
  ...
  const hasNote = title.trim().length > 0 || content.trim().length > 0 || source.length > 0;
